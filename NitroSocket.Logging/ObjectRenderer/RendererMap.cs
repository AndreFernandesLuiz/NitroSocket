...
namespace Library.Logging.ObjectRenderer
{
	/// <summary>
			...
			else 
			{
				...
				else
				{
					...
					catch(Exception ex)
					{
						...
						Library.Logging.Util.LogLog.Error("RendererMap: Exception while rendering object of type ["+obj.GetType().FullName+"]", ex);
						...
						writer.Write("<Library.Logging.Error>Exception rendering object type ["+objectTypeName+"]");

                        writer.Write("<stackTrace>" + ex.ToString() + "</stackTrace>");
						
						writer.Write("</Library.Logging.Error>");
					}
				}
			}
		}

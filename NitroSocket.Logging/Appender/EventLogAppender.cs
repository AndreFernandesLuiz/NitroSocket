...
namespace Library.Logging.Appender
{
	...
	/// </code>
	/// <para>
    /// The Level is the standard  Logging logging level and eventLogEntryType can be any value
	...
	public class EventLogAppender : AppenderSkeleton
	{
		...
		override public void ActivateOptions() 
		{
			...
			{
				LogLog.Trace("EventLogAppender: Changing event source [" + m_applicationName + "] from log [" + currentLogName + "] to log [" + m_logName + "]");
			}
			else if (!sourceAlreadyExists)
			{
				LogLog.Trace("EventLogAppender: Creating event source Source [" + m_applicationName + "] in log " + m_logName + "]");
			}
			...
			LogLog.Trace("EventLogAppender: Source [" + m_applicationName + "] is registered to log [" + registeredLogName + "]");		
		}
		...
		override protected void Append(LoggingEvent loggingEvent) 
		{
			...
			try
			{
				...
				using(SecurityContext.Impersonate(this))
				{
                    ...
                    EventLog.WriteEntry(m_applicationName, eventTxt, entryType, loggingEvent.EventID);
				}
			}
			catch(Exception ex)
			{

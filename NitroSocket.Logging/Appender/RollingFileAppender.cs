...
namespace Library.Logging.Appender
{
	...
	public class RollingFileAppender : FileAppender
	{
		...
		/// <see cref="Library.Logging.DateFormatter.SimpleDateFormatter" />.
		/// </para>
		/// <para>
		...
		private void DetermineCurSizeRollBackups()
		{
			...
			LogLog.Trace("RollingFileAppender: curSizeRollBackups starts at ["+m_curSizeRollBackups+"]");
		}
		...
		private ArrayList GetExistingFiles(string baseFilePath)
		{
			...
			LogLog.Trace("RollingFileAppender: Searched for existing files in ["+directory+"]");
			return alFiles;
		}
		...
		private void RollOverIfDateBoundaryCrossing()
		{
			if (m_staticLogFileName && m_rollDate) 
			{
				if (FileExists(m_baseFileName)) 
				{
					...
					LogLog.Trace("RollingFileAppender: ["+last.ToString(m_datePattern,System.Globalization.DateTimeFormatInfo.InvariantInfo)+"] vs. ["+m_now.ToString(m_datePattern,System.Globalization.DateTimeFormatInfo.InvariantInfo)+"]");
					...
					if (!(last.ToString(m_datePattern,System.Globalization.DateTimeFormatInfo.InvariantInfo).Equals(m_now.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo)))) 
					{
						...
						LogLog.Trace("RollingFileAppender: Initial roll over to ["+m_scheduledFilename+"]");
						RollOverTime(false);
						LogLog.Trace("RollingFileAppender: curSizeRollBackups after rollOver at ["+m_curSizeRollBackups+"]");
					}
				}
			}
		}
		...
		protected void ExistingInit() 
		{
			...
			if (AppendToFile == false)
			{
				...
				if (fileExists)
				{
					...
					{
						LogLog.Trace("RollingFileAppender: Output file ["+fileName+"] already exists. MaxSizeRollBackups is 0; cannot roll. Overwriting existing file.");
					}
					else
					{
						LogLog.Trace("RollingFileAppender: Output file ["+fileName+"] already exists. Not appending to file. Rolling existing file out of the way.");
		...
		private void InitializeFromOneFile(string baseFile, string curFileName)
		{
			...
			if (m_rollDate && !m_staticLogFileName)
			{
				...
				{
					LogLog.Trace("RollingFileAppender: Ignoring file ["+curFileName+"] because it is from a different date period");
					return;
				}
			}
			...
			try 
			{
				...
				if (SystemInfo.TryParse(curFileName.Substring(index + 1), out backup))
				{
					if (backup > m_curSizeRollBackups)
					{
						...
						LogLog.Trace("RollingFileAppender: File name ["+curFileName+"] moves current count to ["+m_curSizeRollBackups+"]");
					}
				}
			} 
			catch(FormatException) 
			{
				...
				LogLog.Trace("RollingFileAppender: Encountered a backup file not ending in .x ["+curFileName+"]");
			}
		}
		...
		private RollPoint ComputeCheckPeriod(string datePattern) 
		{
			...
			for(int i = (int)RollPoint.TopOfMinute; i <= (int)RollPoint.TopOfMonth; i++) 
			{
				...
				LogLog.Trace("RollingFileAppender: Type = ["+i+"], r0 = ["+r0+"], r1 = ["+r1+"]");
		...
		protected void RollFile(string fromFile, string toFile) 
		{
			if (FileExists(fromFile))
			{
				...
				try
				{
					LogLog.Trace("RollingFileAppender: Moving [" + fromFile + "] -> [" + toFile + "]");
					using(SecurityContext.Impersonate(this))
					{
		...
		protected void DeleteFile(string fileName) 
		{
			if (FileExists(fileName)) 
			{
				...
				{
					LogLog.Trace("RollingFileAppender: Exception while moving file to be deleted [" + fileName + "] -> [" + tempFileName + "]", moveEx);
				}
				...
				try
				{
					...
					LogLog.Trace("RollingFileAppender: Deleted file [" + fileName + "]");
				}
				catch(Exception deleteEx)
				{
					...
					else
					{
						...
						LogLog.Trace("RollingFileAppender: Exception while deleting temp file [" + fileToDelete + "]", deleteEx);
					}
				}
			}
		}
		...
		protected void RollOverSize() 
		{
			...
			LogLog.Trace("RollingFileAppender: rolling over count ["+((CountingQuietTextWriter)QuietWriter).Count+"]");
			LogLog.Trace("RollingFileAppender: maxSizeRollBackups ["+m_maxSizeRollBackups+"]");
			LogLog.Trace("RollingFileAppender: curSizeRollBackups ["+m_curSizeRollBackups+"]");
			LogLog.Trace("RollingFileAppender: countDirection ["+m_countDirection+"]");

...
namespace Library.Logging.Config
{
	...
	/// <note>
	/// <para>
	///  Logging will only look for assembly level configuration attributes once.
	/// When using the  Logging assembly level attributes to control the configuration 
	/// of  Logging you must ensure that the first call to any of the 
	/// <see cref="Library.Logging.Core.LoggerManager"/> methods is made from the assembly with the configuration
	/// attributes. 
	/// </para>
	/// <para>
	/// If you cannot guarantee the order in which  Logging calls will be made from 
	...
	public /*sealed*/ class XmlConfiguratorAttribute : ConfiguratorAttribute
	{
		...
		override public void Configure(Assembly sourceAssembly, ILoggerRepository targetRepository)
		{
			...
			catch
			{
                ...
                LogLog.Warn("Exception while trying to get the base directory for AppDomain");
			}

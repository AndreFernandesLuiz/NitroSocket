...
{
	/// <summary>
    /// Use this class to initialize the  Logging environment using an Xml tree.
	/// </summary>
	/// <remarks>
	/// <para>
	...
	public sealed class XmlConfigurator
	{
		...
		/// <summary>
        /// Automatically configures the  Logging system based on the 
        ...
        /// <c> Logging</c> that contains the configuration data.
		/// </para>
		/// <para>
        /// To use this method to configure  Logging you must specify 
        /// the <see cref=" LoggingConfigurationSectionHandler"/> section
        /// handler for the <c> Logging</c> configuration section. See the
		...
		{
			LogLog.Trace("XmlConfigurator: configuring repository [" + repository.Name + "] using .config file section");

			try
			{
				LogLog.Trace("XmlConfigurator: Application config file is [" + SystemInfo.ConfigurationFileLocation + "]");
			}
			catch
			{
				// ignore error
				LogLog.Trace("XmlConfigurator: Application config file location unknown");
			}
			...
			try
			{
                ...
                // hard coded section
                configElement = ConfigurationManager.GetSection("ApplicationConfig/Logging") as XmlElement;
				if (configElement == null)
				{
					// Failed to load the xml config using configuration settings handler
                    LogLog.Error("XmlConfigurator: Failed to find configuration section ' Logging' in the application's .config file. Check your .config file for the < Logging> and <configSections> elements. The configuration section should look like: <section name=\" Logging\" type=\"Library.Logging.Config. LoggingConfigurationSectionHandler, Logging\" />");
				}
				else
				{
			...
			catch(System.Configuration.ConfigurationException confEx)
			{
				...
				else
				{
                    ...
                    string configSectionStr = "<section name=\"Library.Logging\" type=\"Library.Logging.Config. LoggingConfigurationSectionHandler," + Assembly.GetExecutingAssembly().FullName + "\" />";
		...
		/// <summary>
        /// Configures  Logging using a <c> Logging</c> element
		/// </summary>
		/// <remarks>
		/// <para>
        /// Loads the  Logging configuration from the XML element
		...
		{
			LogLog.Trace("XmlConfigurator: configuring repository [" + repository.Name + "] using XML element");
...
#if !NETCF
		/// <summary>
        /// Configures  Logging using the specified configuration file.
		/// </summary>
        ...
        /// the  Logging configuration data.
		/// </para>
		/// <para>
        /// The  Logging configuration file can possible be specified in the application's
        ...
        /// a configuration section for the <c> Logging</c> element otherwise .NET will 
		...
		/// </para>
		/// <example>
        /// The following example configures  Logging using a configuration file, of which the 
		...
		/// ...
		/// 
        /// XmlConfigurator.Configure(new FileInfo(ConfigurationSettings.AppSettings[" Logging-config-file"]));
		/// </code>
		/// <para>
        /// In the <c>.config</c> file, the path to the  Logging can be specified like this :
		/// </para>
		/// <code lang="XML" escaped="true">
		/// <configuration>
		///		<appSettings>
        ///			<add key=" Logging-config-file" value="log.config"/>
		///		</appSettings>
		///	</configuration>
		/// </code>
		/// </remarks>
		...
		/// <summary>
        /// Configures  Logging using the specified configuration URI.
		/// </summary>
		...
		/// <summary>
        /// Configures  Logging using the specified configuration data stream.
		/// </summary>
		...
		{
			LogLog.Trace("XmlConfigurator: configuring repository [" + repository.Name + "] using file [" + configFile + "]");
			...
			else
			{
				...
				{
					LogLog.Trace("XmlConfigurator: config file [" + configFile.FullName + "] not found. Configuration unchanged.");
				}
			}
		}
		...
		{
			LogLog.Trace("XmlConfigurator: configuring repository [" + repository.Name + "] using stream");
			...
			else
			{
				...
				if (doc != null)
				{
					LogLog.Trace("XmlConfigurator: loading XML configuration");

                    // Configure using the ' Logging' element
                    XmlNodeList configNodeList = doc.GetElementsByTagName("Logging");
					if (configNodeList.Count == 0)
					{
                        LogLog.Trace("XmlConfigurator: XML configuration does not contain a < Logging> element. Configuration Aborted.");
					}
		...
		/// <summary>
        /// Configures  Logging using the file specified, monitors the file for changes 
		...
		/// </para>
		/// <para>
        /// For more information on how to configure  Logging using
		...
		{
			LogLog.Trace("XmlConfigurator: configuring repository [" + repository.Name + "] using file [" + configFile + "] watching for file updates");
		...
		private sealed class ConfigureAndWatchHandler
		{
			...
			{
				LogLog.Trace("ConfigureAndWatchHandler: "+e.ChangeType+" [" + m_configFile.FullName + "]");
		...
		/// <summary>
        /// Configures the specified repository using a <c> Logging</c> element.
		/// </summary>
		...
		static private void ConfigureFromXml(ILoggerRepository repository, XmlElement element) 
		{
			...
			{
				LogLog.Trace("XmlConfigurator: Configuring Repository [" + repository.Name + "]");

...
namespace Library.Logging
{
	...
	public sealed class LogManager
	{
		...
		{
            // Between the generation of 4.1.0.0 and 4.2.0.0 this library stopped working without any evidence
            // after searching on the web, we've found the solution described on the link bellow
            // Apparently the method call GetRepository() solve the problem, we decided to keep the configure 
            // method on the case of the GetRopsitory return a really false value.
            //
            // http://www.mail-archive.com/log4net-user@logging.apache.org/msg03979.html
            //
            if (LogManager.GetRepository().Configured == false)
            {
                if (Log.CallStatementConfigure)
                {
                    Library.Logging.Config.XmlConfigurator.Configure();
                    //System.Diagnostics.EventLog.WriteEntry("Library.Logging", "Configuring settings - XmlConfigurator", System.Diagnostics.EventLogEntryType.Warning);
                }
            }

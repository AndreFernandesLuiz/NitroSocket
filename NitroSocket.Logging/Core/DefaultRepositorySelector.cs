...
namespace Library.Logging.Core
{
	...
	public class DefaultRepositorySelector : IRepositorySelector
	{
		...
		/// an appropriate value would be <see cref="Library.Logging.Repository.Hierarchy.Hierarchy"/>.
		/// </para>
		/// </remarks>
		...
		public DefaultRepositorySelector(Type defaultRepositoryType)
		{
			...
			{
                throw Library.Logging.Util.SystemInfo.CreateArgumentOutOfRangeException("defaultRepositoryType", defaultRepositoryType, "Parameter: defaultRepositoryType, Value: [" + defaultRepositoryType + "] out of range. Argument must implement the ILoggerRepository interface");
			}

			m_defaultRepositoryType = defaultRepositoryType;

			LogLog.Trace("DefaultRepositorySelector: defaultRepositoryType [" + m_defaultRepositoryType + "]");
		}
		...
		/// to create can be overridden by specifying the <see cref="Library.Logging.Config.RepositoryAttribute"/> 
		/// attribute on the <paramref name="repositoryAssembly"/>.
		/// </para>
		/// <para>
		/// The default values are to use the <see cref="Library.Logging.Repository.Hierarchy.Hierarchy"/> 
		...
		/// any <see cref="Library.Logging.Config.ConfiguratorAttribute"/> attributes defined on
		...
		/// <see cref="Library.Logging.Config.RepositoryAttribute"/> attribute on the 
		...
		/// configured using any <see cref="Library.Logging.Config.ConfiguratorAttribute"/> 
		...
		/// Also the <see cref="Library.Logging.Config.RepositoryAttribute"/> attribute on the
		...
		public ILoggerRepository CreateRepository(Assembly repositoryAssembly, Type repositoryType, string repositoryName, bool readAssemblyAttributes)
		{
			...
			lock(this)
			{
				...
				if (rep == null)
				{
					...
					LogLog.Trace("DefaultRepositorySelector: Creating repository for assembly [" + repositoryAssembly + "]");
					...
					LogLog.Trace("DefaultRepositorySelector: Assembly [" + repositoryAssembly + "] using repository [" + actualRepositoryName + "] and repository type [" + actualRepositoryType + "]");
					...
					{
						LogLog.Trace("DefaultRepositorySelector: repository [" + actualRepositoryName + "] already exists, using repository type [" + rep.GetType().FullName + "]");
				...
				else
				{
					...
					if (aliasedRepository != null)
					{
						...
						if (aliasedRepository.GetType() == repositoryType)
						{
							...
							LogLog.Trace("DefaultRepositorySelector: Aliasing repository [" + repositoryName + "] to existing repository [" + aliasedRepository.Name + "]");
					...
					if (rep == null)
					{
						LogLog.Trace("DefaultRepositorySelector: Creating repository [" + repositoryName + "] using type [" + repositoryType + "]");
		...
		/// </summary>
		/// <param name="assembly">The assembly that has a <see cref="Library.Logging.Config.RepositoryAttribute"/>.</param>
		...
		private void GetInfoForAssembly(Assembly assembly, ref string repositoryName, ref Type repositoryType)
		{
			...
			try
			{
                ...
                object[] repositoryAttributes = Attribute.GetCustomAttributes(assembly, typeof(Library.Logging.Config.RepositoryAttribute), false);
				if (repositoryAttributes == null || repositoryAttributes.Length == 0)
				{
					// This is not a problem, but its nice to know what is going on.
					LogLog.Trace("DefaultRepositorySelector: Assembly [" + assembly + "] does not have a RepositoryAttribute specified.");
				}
				else
				{
					if (repositoryAttributes.Length > 1)
					{
						LogLog.Error("DefaultRepositorySelector: Assembly [" + assembly + "] has multiple Library.Logging.Config.RepositoryAttribute assembly attributes. Only using first occurrence.");
					}

                    Library.Logging.Config.RepositoryAttribute domAttr = repositoryAttributes[0] as Library.Logging.Config.RepositoryAttribute;
		...
		/// </summary>
		/// <param name="assembly">The assembly containing <see cref="Library.Logging.Config.ConfiguratorAttribute"/>
		...
		private void ConfigureRepository(Assembly assembly, ILoggerRepository repository)
		{
            ...
            object[] configAttributes = Attribute.GetCustomAttributes(assembly, typeof(Library.Logging.Config.ConfiguratorAttribute), false);
			if (configAttributes != null && configAttributes.Length > 0)
			{
                ...
                foreach (Library.Logging.Config.ConfiguratorAttribute configAttr in configAttributes)
				{
					if (configAttr != null)
					{
						try
			...
			if (repository.Name == DefaultRepositoryName)
			{
				...
				string repositoryConfigFile = SystemInfo.GetAppSetting("Library.Logging.Config");
				if (repositoryConfigFile != null && repositoryConfigFile.Length > 0)
				{
					...
					{
						LogLog.Warn("DefaultRepositorySelector: Exception getting ApplicationBaseDirectory. appSettings Library.Logging.Config path ["+repositoryConfigFile+"] will be treated as an absolute URI", ex);
					}
					...
					{
						LogLog.Error("DefaultRepositorySelector: Exception while parsing Library.Logging.Config file path ["+repositoryConfigFile+"]", ex);
					}

					if (repositoryConfigUri != null)
					{
						LogLog.Trace("DefaultRepositorySelector: Loading configuration for default repository from AppSettings specified Config URI ["+repositoryConfigUri.ToString()+"]");
		...
		private void LoadPlugins(Assembly assembly, ILoggerRepository repository)
		{
            ...
            object[] configAttributes = Attribute.GetCustomAttributes(assembly, typeof(Library.Logging.Config.PluginAttribute), false);
			if (configAttributes != null && configAttributes.Length > 0)
			{
                foreach (Library.Logging.Plugin.IPluginFactory configAttr in configAttributes)
				{
					try
					{
		...
		private void LoadAliases(Assembly assembly, ILoggerRepository repository)
		{
            ...
            object[] configAttributes = Attribute.GetCustomAttributes(assembly, typeof(Library.Logging.Config.AliasRepositoryAttribute), false);
			if (configAttributes != null && configAttributes.Length > 0)
			{
                foreach (Library.Logging.Config.AliasRepositoryAttribute configAttr in configAttributes)
				{
					try
					{
		...
		private const string DefaultRepositoryName = " Logging-default-repository";

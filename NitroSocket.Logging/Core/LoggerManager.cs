...
namespace Library.Logging.Core
{
	...
	/// </para>
	/// <para>
	/// This class is used by the wrapper managers (e.g. <see cref="Library.Logging.LogManager"/>)
	...
	/// the <see cref="RepositorySelector"/> property, or by setting the <c>Library.Logging.RepositorySelector</c>
	...
	public sealed class LoggerManager
	{
		...
		/// These are used to shutdown the  Logging system as the application exits.
		/// </para>
		/// </remarks>
		static LoggerManager()
		{
			...
			{
				LogLog.Trace("LoggerManager: Security Exception (ControlAppDomain LinkDemand) while trying "+
			...
			// Dump out our assembly version into the log if trace is enabled
			LogLog.Trace(GetVersionInfo());

			// Set the default repository selector
#if NETCF
			s_repositorySelector = new CompactRepositorySelector(typeof(Library.Logging.Repository.Hierarchy.Hierarchy));
#else

			// Look for the RepositorySelector type specified in the AppSettings 'Library.Logging.RepositorySelector'
			string appRepositorySelectorTypeName = SystemInfo.GetAppSetting("Library.Logging.RepositorySelector");
			...
			{
                s_repositorySelector = new DefaultRepositorySelector(typeof(Library.Logging.Repository.Hierarchy.Hierarchy));
			}
#endif
		}
		...
		/// of the root logger to <see cref="Level.Trace"/>. Moreover,
		...
		/// <see cref="Library.Logging.Repository.Hierarchy.Hierarchy"/> object.
		/// </para>
		/// <para>
		...
		private static string GetVersionInfo()
		{
			...
			// Build Up message
			sb.Append(" Logging assembly [").Append(myAssembly.FullName).Append("]. ");
		...
		/// </para>
		/// <para>
		/// When the event is triggered the  Logging system is <see cref="Shutdown()"/>.
		/// </para>
		/// </remarks>

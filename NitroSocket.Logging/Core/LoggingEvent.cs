...
namespace Library.Logging.Core
{
    ...
    /// is passed around to the different  Logging components.
	/// </para>
	/// <para>
	/// This class is of concern to those wishing to extend Library.Logging.
	/// </para>
	/// <para>
	...
	public class LoggingEvent : ISerializable
	{
		...
		/// </para>
        /// <para>This method is called by the  Logging framework
		/// to create a logging event.
		/// </para>
		/// </remarks>
        public LoggingEvent(Type callerStackBoundaryDeclaringType, Library.Logging.Repository.ILoggerRepository repository, string loggerName, int eventId, Level level, object message, Exception exception) 
		{
        ...
        /// to be created independently of the  Logging framework. This can
		...
		/// </para>
		/// </remarks>
        public LoggingEvent(Type callerStackBoundaryDeclaringType, Library.Logging.Repository.ILoggerRepository repository, LoggingEventData data, FixFlags fixedData) 
		{
			m_callerStackBoundaryDeclaringType = callerStackBoundaryDeclaringType;
			m_repository = repository;
		...
		/// </para>
		/// </remarks>
        public LoggingEvent(Type callerStackBoundaryDeclaringType, Library.Logging.Repository.ILoggerRepository repository, LoggingEventData data)
		...
		protected LoggingEvent(SerializationInfo info, StreamingContext context) 
		{
            ...
            m_data.EventID = info.GetInt32("EventID");
		...
		/// <summary>
		/// Gets the <see cref="EventID" /> of the logging event.
		/// </summary>
		/// <value>
		/// The <see cref="int" /> of the logging event.
		/// </value>
		/// <remarks>
		/// <para>
		/// Gets the <see cref="EventID" /> of the logging event.
		...
		/// supported frameworks and the different behavior in Trace and
		/// Release builds.
		/// </para>
		/// </remarks>
		...
		public void WriteRenderedMessage(TextWriter writer)
		{
            ...
            {
                //Build the message using the exception
                string message = m_thrownException.GetType().Name + " - " + m_thrownException.Message;
                // logging data of exception
                message += EmitErrorLineNested(m_thrownException);

                #region building log InnerException messages
                Exception inner = m_thrownException.InnerException;
                StringBuilder messagesb = new StringBuilder();
                while (inner != null)
                {
                    messagesb.Append("[");
                    messagesb.AppendFormat("{0} - ", inner.GetType().Name);     // class name exception
                    messagesb.AppendFormat("{0} ", inner.Message);              // message exception
                    messagesb.AppendFormat("{0}", EmitErrorLineNested(inner));  // innerexception data 
                    messagesb.Append("]");

                    inner = inner.InnerException;       // get next level of inner exception
                }
				...
				if (m_data.ThreadName == null && this.m_cacheUpdatable)
				{
					...
					if (m_data.ThreadName == null || m_data.ThreadName.Length == 0)
					{
						...
						catch(System.Security.SecurityException)
						{
							...
							LogLog.Trace("LoggingEvent: Security exception while trying to get current thread ID. Error Ignored. Empty thread name.");
			...
			get
			{
				if (m_data.UserName == null  && this.m_cacheUpdatable) 
				{
					...
					catch(System.Security.SecurityException)
					{
						// This security exception will occur if the caller does not have 
						// some undefined set of SecurityPermission flags.
						LogLog.Trace("LoggingEvent: Security exception while trying to get current windows identity. Error Ignored. Empty user name.");
			...
			get
			{
				if (m_data.Identity == null  && this.m_cacheUpdatable)
				{
					...
					catch(System.Security.SecurityException)
					{
						// This security exception will occur if the caller does not have 
						// some undefined set of SecurityPermission flags.
						LogLog.Trace("LoggingEvent: Security exception while trying to get current thread principal. Error Ignored. Empty identity name.");
		...
		/// <see cref="Library.Logging.Appender.IAppender.DoAppend"/> method call if this event 
		...
		public virtual void GetObjectData(SerializationInfo info, StreamingContext context)
		{
            ...
            info.AddValue("EventID", m_data.EventID);
		...
		{
            if (m_data.Level == Level.BusinessError)
            {
            ...
            else
            {
                if (m_data.ExceptionString == null && this.m_cacheUpdatable)
                {
                    if (m_thrownException != null)
                    {
                        ...
                        // Append Exception Data attribute (17/01/2007)
                        //m_data.ExceptionString = EmitErrorLineNested(m_data, m_thrownException);
                    }
                    else
                    {
                        m_data.ExceptionString = "";
		...
		/// </summary>
        public const string HostNameProperty = " Logging:HostName";

		/// <summary>
		/// The key into the Properties map for the thread identity value.
		/// </summary>
        public const string IdentityProperty = " Logging:Identity";

		/// <summary>
		/// The key into the Properties map for the user name value.
		/// </summary>
        public const string UserNameProperty = " Logging:UserName";

		#endregion

        /// <summary>
        /// Writes output to the standard error stream to Data attribute.  
        /// </summary>
        /// <param name="objdataAttribute">Object that content Data attribute</param>

        private string EmitErrorLineNested(Exception paramEx)
        {
            StringBuilder sb = new StringBuilder();

            if ((paramEx != null) && (paramEx.Data.Count > 0))
            {
                foreach (DictionaryEntry dic in paramEx.Data)

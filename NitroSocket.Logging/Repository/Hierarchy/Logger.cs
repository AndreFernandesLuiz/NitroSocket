...
namespace Library.Logging.Repository.Hierarchy
{
	...
	/// </para>
	/// <para>
	/// This is one of the central classes in the  Logging implementation. One of the
	/// distinctive features of  Logging are hierarchical loggers and their
	...
	public abstract class Logger : IAppenderAttachable, ILogger
	{
		...
		virtual public void AddAppender(IAppender newAppender) 
		{
			...
			try
			{
				...
				{
                    m_appenderAttachedImpl = new Library.Logging.Util.AppenderAttachedImpl();
				}
		...
		/// </para>
		/// </remarks>
		virtual public void Log(Type callerStackBoundaryDeclaringType, int eventId, Level level, object message, Exception exception) 
		{
			try
			{
				if (IsEnabledFor(level))
				{
                    ForcedLog((callerStackBoundaryDeclaringType != null) ? callerStackBoundaryDeclaringType : ThisDeclaringType, eventId, level, message, exception);
				}
			}
			catch (Exception ex)
			{
                Library.Logging.Util.LogLog.Error("Log: Exception while logging", ex);
			}
		}

        virtual public void Log(Type callerStackBoundaryDeclaringType, Level level, object message, Exception exception)
        {
            Log(callerStackBoundaryDeclaringType, 0, level, message, exception);
        }
		...
		virtual protected void CallAppenders(LoggingEvent loggingEvent) 
		{
			...
			{
				LogLog.Trace("Logger: No appenders could be found for logger [" + Name + "] repository [" + Repository.Name + "]");
				LogLog.Trace("Logger: Please initialize the  Logging system properly.");
				try
				{
					LogLog.Trace("Logger:    Current AppDomain context information: ");
					LogLog.Trace("Logger:       BaseDirectory   : " + SystemInfo.ApplicationBaseDirectory);
#if !NETCF
					LogLog.Trace("Logger:       FriendlyName    : " + AppDomain.CurrentDomain.FriendlyName);
					LogLog.Trace("Logger:       DynamicDirectory: " + AppDomain.CurrentDomain.DynamicDirectory);
#endif
				}
				catch(SecurityException secEx)
				{
					// Insufficient permissions to display info from the AppDomain
                    LogLog.Warn("Insufficient permissions to display info from the AppDomain: " + secEx.Message);
				}
		...
		/// </para>
		/// </remarks>
		virtual public void Log(int eventId, Level level, object message, Exception exception) 
		{
			if (IsEnabledFor(level))
			{
                ForcedLog(ThisDeclaringType, eventId, level, message, exception);
			}
		}
		...
		/// </para>
		/// </remarks>
        virtual protected void ForcedLog(Type callerStackBoundaryDeclaringType, int eventId, Level level, object message, Exception exception) 
		{
            CallAppenders(new LoggingEvent(callerStackBoundaryDeclaringType, this.Hierarchy, this.Name, eventId, level, message, exception));
		}
		...
		/// </summary>
        private Library.Logging.Util.AppenderAttachedImpl m_appenderAttachedImpl;

...
namespace Library.Logging.Util
{
	...
	public sealed class SystemStringFormat
	{
		...
		private static string StringFormat(IFormatProvider provider, string format, params object[] args)
		{
			...
			{
                Library.Logging.Util.LogLog.Warn("StringFormat: Exception while rendering format [" + format + "]", ex);
		...
		private static string StringFormatError(Exception formatException, string format, object[] args)
		{
			try
			{
				StringBuilder buf = new StringBuilder("<Library.Logging.Error>");
				...
				buf.Append("</args>");
				buf.Append("</Library.Logging.Error>");

				return buf.ToString();
			}
			catch(Exception ex)
			{
                Library.Logging.Util.LogLog.Error("StringFormat: INTERNAL ERROR during StringFormat error handling", ex);
				return "<Library.Logging.Error>Exception during StringFormat. See Internal Log.</Library.Logging.Error>";
			}
		}

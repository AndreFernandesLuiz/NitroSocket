...
namespace Library.Logging.Util
{
	...
	public sealed class SystemInfo
	{
...
#if !NETCF
			// Look for Library.Logging.NullText in AppSettings
			string nullTextAppSettingsKey = SystemInfo.GetAppSetting("Library.Logging.NullText");
			if (nullTextAppSettingsKey != null && nullTextAppSettingsKey.Length > 0)
			{
				LogLog.Trace("SystemInfo: Initializing NullText value to [" + nullTextAppSettingsKey + "].");
				nullText = nullTextAppSettingsKey;
			}

			// Look for Library.Logging.NotAvailableText in AppSettings
			string notAvailableTextAppSettingsKey = SystemInfo.GetAppSetting("Library.Logging.NotAvailableText");
			if (notAvailableTextAppSettingsKey != null && notAvailableTextAppSettingsKey.Length > 0)
			{
				LogLog.Trace("SystemInfo: Initializing NotAvailableText value to [" + notAvailableTextAppSettingsKey + "].");
		...
		public static string HostName
		{
			get
			{
				if (s_hostName == null)
				{
					...
					{
                        LogLog.Warn("Exception while trying to get host name: " + ex.Message);
					}
					...
					if (s_hostName == null || s_hostName.Length == 0)
					{
						...
						{
                            LogLog.Warn("Exception while trying to get machine name:  " + ex.Message);
						}
			...
			get
			{
				if (s_appFriendlyName == null)
				{
					...
					catch(System.Security.SecurityException)
					{
						...
						LogLog.Trace("SystemInfo: Security exception while trying to get current domain friendly name. Error Ignored.");
					}
					...
					if (s_appFriendlyName == null || s_appFriendlyName.Length == 0)
					{
						...
						catch(System.Security.SecurityException ex)
						{
                            ...
                            LogLog.Warn("Exception while trying to get application name: " + ex.Message);
						}
					}
		...
		/// <remarks>
		/// <para>
		/// This is the time at which the  Logging library was loaded into the
		/// AppDomain. Due to reports of a hang in the call to <c>System.Diagnostics.Process.StartTime</c>
		/// this is not the start time for the current process.
		/// </para>
		/// <para>
		/// The  Logging library should be loaded by an application early during its
		...
		/// a value for the <c>Library.Logging.NullText</c> appSetting in the application's
		/// .config file.
		/// </para>
		/// </remarks>
		...
		/// a value for the <c>Library.Logging.NotAvailableText</c> appSetting in the application's
		/// .config file.
		/// </para>
		/// </remarks>
		...
		public static bool TryParse(string s, out int val)
		{
			...
			catch (Exception ex)
			{
                ...
                LogLog.Warn("Insufficient permissions to parse value: " + ex.Message);
